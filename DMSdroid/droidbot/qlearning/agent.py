...
class Agent(object):
        ...
        while right <= len(cur_seqs):
            ...
            for key, value in self.semantic_seqs.items():
                for subseq in value:
                    ...
                    subseq_index = utils.mate_search(subseq, window)
                    if subseq_index != -1:
                        if subseq_index[0] > 0:
                            nseq_len += subseq_index[0] - 1
                        seq_semantic_order.append(key)
                        left = left + subseq_index[1]  # 左指针向右移动，减小窗口
                        break
            # 右指针向右移动, 增大窗口
            right += 1
        nseq_len += right - left - 2
        return [seq_semantic_order, nseq_len]
    ...
    def get_fallible_seq_nums(self, seq_type_order):
        if len(seq_type_order) == 0:
            return
        fallible_seq = [['DELETE', 'CREATE'], ['DELETE', 'READ'], ['CREATE', 'READ']]
        nums = 0
    ...
    def get_semantic_reward(self, seq):
        ...
        order_res = self.get_seq_types(seq)
        self.seq_type_order = order_res[0]
        self.sseq_len = order_res[1]
        if len(self.seq_type_order) == 0 or seq_type_order == self.seq_type_order:
            return 0
        utils.log_event_seq({'type': 'semanticsSeq', 'info': self.seq_type_order}, self.device.output_dir)
    ...
    def updateQ_table(self, state_t, action_t, state_t1, reward):
        ...
        self.check_state_exist(state_t1)
        self.check_state_exist(state_t)
        if len(self.q_table[state_t1.structure_str])==0:
            target=50
        else:
    ...
    def get_beta_reward(self, action_t, state_t):
        keywordlist = ['add', 'delete', 'edit', 'modify', 'ok', 'confirm', 'new', 'create', '+', 'remove', 'rename',
                     'save', 'yes', 'search', 'find', 'update', 'done']
        print(action_t.view['resource_id'])
        id_name = action_t.view['resource_id']
        if id_name==None:
            return 0
        if any(x.lower() in id_name.lower() for x in keywordlist):
            reward = 1
        else:
            reward = 0
        return reward
...
if __name__ == "__main__":
    ...
    print('get seq')
    cur_seqs = ['A','B','C','F','H','A','D','F','E','J','B','G','H','F','B','A','C','C','D']
    seq_semantic_order = []
    # 定义初始窗口大小
    window_size = 2
    # 双指针初始化
    ...
    while right <= len(cur_seqs):
        ...
        window = cur_seqs[left:right]
        # 遍历语义序列，判断当前窗口是否符合语义序列中的任意一个子序列
        for key, value in semantic_seqs.items():
            for subseq in value:
                subseq_index = utils.mate_search(subseq, window)
                if subseq_index != -1:
                    if subseq_index[0]>0:
