    ...
    def start(self):
        ...
        while time.time() - self.start_time <= STAGE_ONE_TIME:
            ...
            if action_t == None:
                ...
                seq = [self.generate_state_semantic_str(state_t, action_t)]
                test_case = []
            # step2: execute action12eqw
            utils.log_event_seq({'type': 'event', 'info': action_t}, outdir=self.device.output_dir)
            ...
            # beta step4
            # semantic_reward_beta = self.agent.get_beta_reward(action_t, state_t)
            # print('semantic_reward_beta', semantic_reward_beta)
            self.agent.updateQ_table(state_t, action_t, state_t1, semantic_reward)
