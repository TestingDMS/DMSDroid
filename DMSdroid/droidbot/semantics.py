...
class Configuration(object):
    # semantic setting
    SEMANTIC_TYPE = ['create', 'delete', 'update', 'search', 'read']
    # 构造规则列表字典
    RULE_DICT = {
        'CREATE': [{'state': 'listView', 'action': [['touch', 'long-touch'], 'add_btn']},
                {'state': 'inputView', 'action': [['set_text', 'touch'], 'inputView']},
                {'state': 'inputView', 'action': [['touch', 'long-touch'], 'submit_btn']}
                ],
        'ADD': [{'state': 'textView', 'action': [['touch', 'long-touch'], 'labelled_btn']}],
        ...
        'UPDATE': [{'state': 'textView', 'action': [['touch', 'long-touch'], 'edit_btn']},
                 {'state': 'inputView', 'action': ['set_text', 'inputView'], 'get_content': True},
                 {'state': 'inputView', 'action': [['touch', 'long-touch'], 'submit_btn']}
                 ],
        'READ': [{'state': 'listView', 'action': [['touch', 'long-touch'], 'listItem_btn'], 'get_content': True},
    ...
    BTN_TYPES = {
        ...
        'delete_btn': ['remove', 'delete', 'clear', 'del'],
        'search_btn': ['search', 'find', 'filter', 'look up'],
        'submit_btn': ['save', 'ok', 'submit', 'yes', 'confirm', 'done', 'continue'],
    }
...
class SemanticsEvents(object):
    ...
    def first_semantic_page(self, widget_type_list, appState, appAction):
        ...
        if appAction.event_type == 'touch' and appAction.element_type == 'add_btn':
            print(widget_type_list['md5Value'] + ' may be the first state of create state')
            cur_eventlist = {"type": "CREATE", "list": [{'state': widget_type_list['md5Value'], 'action': appAction.view['view_str'], 'event_str': appAction.get_event_str(appState)}], "state": 1}
        # 修改
            ...
            cur_eventlist = {"type": "UPDATE", "list": [{'state': widget_type_list['md5Value'], 'action': appAction.view['view_str'], 'event_str': appAction.get_event_str(appState)}], "state": 1}
        ...
        # 进入详情
            ...
            cur_eventlist = {"type": "READ", "list": [{'state': widget_type_list['md5Value'], 'action': appAction.view['view_str'], 'event_str': appAction.get_event_str(appState)}], "state": 1}
        else:
            return None
    ...
    def check_semantic_state(self, appState_t1):
        ...
        if semantic_type == 'SEARCH' or semantic_type == 'DELETE':
            ...
            self.prev_eventlist = {}
        elif semantic_type == 'CREATE' or semantic_type == 'UPDATE':
            if self.content_in_all(content_check, appState_t1):
                # 满足规则，写入txt文件中
                self.save2dir(self.prev_eventlist, semantic_type)
                self.prev_eventlist = {}
        elif semantic_type == 'READ':
            if self.content_in_all(content_check, appState_t1):
                self.save2dir(self.prev_eventlist, semantic_type)
                self.prev_eventlist = {}
        else:
