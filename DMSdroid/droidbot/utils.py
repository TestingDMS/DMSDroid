...
def mate_search(pattern, text):
    n= len(text)
    m= len(pattern)
    i, j = 0, 0
    res=[0,0]
    while i < n:
        if pattern[j] == text[i] and j==(m-1):
            res[1]=i
            return res
        elif pattern[j] == text[i] and j==0:
            res[0] = i
            i += 1
            j += 1
...
def log_event_seq(msg, outdir):
    time = datetime.datetime.now()
    type = msg['type']
    info = msg['info']
    logline = f'{time} {type} {info}'
    ses_outdir = outdir+'\\seslog.txt'
    with open(ses_outdir, 'a') as f:
        f.write(logline)
        f.write('\n')
    f.close()
    return 1
